...
export default function Buscar() {
  ...
  const searchSubmitHandler = () => {
    if (search === "") {
      return Alert.alert("Validación", "Se requiere el nombre de una ciudad", [
        { text: "OK" },
      ]);
    }
  ...
  async function guardarStorage() {
    ...
    if (ciudad != "") {
      if (ciudades.length != 0) {
        for (var i = 0; i < ciudades.length; i++) {
          try {
            ...
            if (valor[0] == ciudad) {
              return Alert.alert("Error", "La ciudad ya está guardada", [
                { text: "OK" },
              ]);
            }
          } catch (err) {
      ...
      try {
        ...
        return Alert.alert("Error", "Error al guardar en favoritos", [
          { text: "OK" },
        ]);
      }
    } else
      return Alert.alert("Error", "No se puede guardar campo vacío", [
        { text: "OK" },
      ]);
  }
